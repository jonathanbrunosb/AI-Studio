...
export default async function StudioPage({ searchParams }: { searchParams: Promise<{ id?: string; category?: string; template?: string; saved?: string; copied?: string }> }) {
  ...
  return <div><PageHeader eyebrow="Produção editorial" title={readOnly ? "Consultar material" : content ? "Editar material" : "Novo material"} description="Organize o conteúdo e personalize sua composição com a identidade da Contabilidade." actions={<>{content && !readOnly && <Link className="primary-button" href={`/studio/editor/${content.id}`}>Abrir editor visual</Link>}<Link className="secondary-button" href="/biblioteca">Voltar à biblioteca</Link></>} />
