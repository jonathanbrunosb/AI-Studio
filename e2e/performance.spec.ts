...
test("linha de base de desempenho (mediana de 5 carregamentos por página)", async ({ page }) => {
  ...
  for (const path of pages) {
    ...
    result[path] = {
      ...
      loadMs: median(samples.map((s) => s.load)), transferKbPrimeiroAcesso: Math.round(samples[0].transferKb),
    };
  }
  const health: number[] = [];
  ...
  const report = {
    ...
    amostrasPorPagina: RUNS, paginas: result, apiHealthMs: { mediana: median(health), max: Math.max(...health) },
  };
